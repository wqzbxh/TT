  ...
  btnHandleCommit = async () => {
    ...
    if (!isNotEmpty(companyName)) {
      this.setState((prevState) => ({
        listMsg: [
          ...prevState.listMsg,
          { msg: "companyName Can not be empty", color: "#F44336" },
        ],
      }));
      return false;
    }
    ...
    const formData = id_companySettings ?  { ...originalData, id: id_companySettings } :originalData;
    const methodType = id_companySettings ? "PUT" : "POST" ;

    const response = await regCompanySeting(formData,methodType);
    const result = response.data;
    if (result.code === 200) {
      this.setState((prevState) => ({
        listMsg: [...prevState.listMsg, { msg: result.msg, color: "#45C468" }],
        id_companySettings: result.data.id
      }));
  ...
  ajaxCurrentList = async () => {
    ...
    if (result.code === 200 && !Array.isArray(result.data)) {
      ...
      this.setState({
        ...
        logo: result.data.companyLogoPath,
        companyPostalCode: result.data.companyPostalCode ?  result.data.companyPostalCode :'',
        companyCity: result.data.companyCity,
        companyStreet1: result.data.companyStreet1,
        companyStreet2: result.data.companyStreet2,
        id_companySettings: result.data.id_companysettings,
...
btnHandleEdit = () => {
  ...
  render() {
    const { logo, is_edit,statusIoc,companyName, companyPostalCode, companyStreet1,companyCity,companyStreet2, edit_text, listMsg } = this.state;
    return (
            ...
            <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
              <div className="col-span-full">
                ...
                <div className="mt-2 flex items-center gap-x-3">
                  <div>
                    <img src={logo} alt="" className="cursor-pointer rounded-full h-24 w-24"   onClick={this.handleUploadClick} />
                  </div>
                  <input
                    type="file"
                    disabled={is_edit}
              ...
              <div className="col-span-full">
                ...
                <div className="mt-2">
                  <input
                    type="text"
                    name="companyStreet1"
                    value={companyStreet1}
                    onChange={this.handelChange}
                    disabled={is_edit}
                    id="companyStreet1"
                    autoComplete="companyStreet1s"
                    className="block w-full rounded-md border-0 py-1.5 pl-2 text-customize-dark-19 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-customize-dark-19 sm:text-sm sm:leading-6"
                  />
                </div>
              </div>
              ...
              <div className="col-span-full">
                ...
                <div className="mt-2">
                  <input
                    ...
                    name="companyStreet2"
                    onChange={this.handelChange}
                    value={companyStreet2}
                    disabled={is_edit}
                    id="companyStreet2"
                    autoComplete="companyStreet2"
                    className="block w-full rounded-md border-0 py-1.5 pl-2  text-customize-dark-19 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-customize-dark-19 sm:text-sm sm:leading-6"
                  />
                </div>
              </div>
